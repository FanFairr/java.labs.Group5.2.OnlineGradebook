...
;
Alter table Student add  foreign key (id_person) references Person (id_person)
;
Alter table Teacher add  foreign key (id_person) references Person (id_person)
;
Alter table Mark add  foreign key (id_student) references Student (id_person)
;
Alter table Mark add  foreign key (id_teacher) references Teacher (id_person)
;
Alter table student_subject add  foreign key (id_person) references Student (id_person)
;
Alter table teacher_subject add  foreign key (id_person) references Teacher (id_person)
;
Alter table teacher_subject add  foreign key (id_subject) references Subject (id_subject)
;
Alter table student_subject add  foreign key (id_subject) references Subject (id_subject)
;
Alter table Task add  foreign key (id_subject) references Subject (id_subject)
;
Alter table Mark add  foreign key (task_id) references Task (task_id)
;
create or replace trigger insertP
...
;
create or replace trigger insertPerson
after insert on PERSON
for each row
declare status_peson VARCHAR2(10);
begin
  select status into status_peson from PERSON1 where LOGIN = :new.LOGIN;
